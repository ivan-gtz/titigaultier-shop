...
const getPayPalBearerToken = async (): Promise<string | null> => {
    ...
    try {
        const result = await fetch(oauth2Url, {...requestOptions, cache: 'no-store'}).then(resp => resp.json());
...
const verifyPayPalPayment = async (paypalTransactionId: string, bearedToken: string): Promise<PayPalOrderStatusResponse | null> => {
