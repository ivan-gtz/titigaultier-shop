...
export const Pagination = ({ totalPages }: Props) => {
    ...
    return (
        <div className="max-w-screen-xl mx-auto mt-12 px-4 text-gray-600 md:px-8 mb-32">
            <div className="hidden items-center justify-between sm:flex" aria-label="Pagination">
                <Link href={ createPageUrl( currentPage - 1 ) } className="hover:text-indigo-600 flex items-center gap-x-2">
                ...
                <ul className="flex items-center gap-1">
                    {
                        allPages.map(( page, idx ) => (
                            <li key={ page + '-' + idx } className="text-sm">
                                {
                                    page == "..." ? (
                                        <div>
                                            {page}
                                        ...
                                        <Link href={ createPageUrl( page )} aria-current={currentPage == page ? "page" : false} className={`px-3 py-2 rounded-lg duration-150 hover:text-indigo-600 hover:bg-indigo-50 ${currentPage == page ? "bg-indigo-50 text-indigo-600 font-medium" : ""}`}>
                                            {page}
                                        </Link>
                                    )
                                }
